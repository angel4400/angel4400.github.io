 ...
 */
function User(userName, userPassword,txtCompanyName) {
...
function login(userName, userPassword) {
    ...
    for (let userStr of userArr) { //遍历数组
        ...
        if (user.userName == userName && user.userPassword == userPassword) { //判断用户名和密码是否匹配
            return true; //匹配成功
        }
    }
    return false; //匹配失败
...
function validate() { //验证验证码是否正确
    var input = document.getElementById("text").value.toUpperCase(); //取得输入的验证码并转化为大写
    if (input == 0) { //若输入为空，则重新获取验证码
        alert("请输入验证码");
    } else if (input != code) {  //若输入的验证码与生成的验证码不一致
        alert("验证码不正确，请重新输入");
        input = " ";
        createCode(); //重新获取验证码
    } else {
 ...
 *    else 注册失败
 */
document.getElementById("btnReg").onclick = function () { //注册按钮绑定onclick事件
    ...
    if (result == 1) { //判断方法的返回值，1：注册成功，转到登录页面
        alert("注册成功"); //弹出提示
